...
async function checkAuth() {
  try {
    ...
    if (error || !data.session) {
      ...
      // Задай currentEmail и currentUser при refresh
      currentEmail = data.session.user.email;
      currentUser = Object.keys(USER_MAP).find(k => USER_MAP[k] === currentEmail) || '';
      // Пренасочи Лука към boss.html
...
async function logout() {
  ...
  if (btnEl)   { btnEl.disabled = false; btnEl.textContent = 'Влез'; }
...
// ── DESKTOP / VISUAL NOTIFICATION ──
function showDesktopNotification(order) {
  if (!('Notification' in window) || Notification.permission !== 'granted') return;
  try {
    const n = new Notification('🔔 Нова поръчка ' + order.order_number, {
      body: `${order.name} · €${order.total}\n${order.delivery_type === 'delivery' ? '🛵 Доставка' : '🚶 Вземане'} · ${order.pickup_time}`,
...
function activateSound() {
  try {
    ...
    // Request desktop notification permission too
    ...
    if (banner) {
      ...
      banner.innerHTML = '<span style="font-size:0.82rem;color:#2ed573;">✅ Активирано — ще получаваш звук и известие на екрана при нова поръчка</span>';
...
async function submitShift() {
  ...
  const cardAmount = parseFloat(document.getElementById('shiftCardAmount').value) || 0;
  const note   = document.getElementById('shiftNote').value.trim();
  const date   = document.getElementById('shiftDate').value;
  if (!date)   { showToastAdmin('⚠️ Избери дата!'); return; }
  if (!amount || amount <= 0) { showToastAdmin('⚠️ Въведи обща сума!'); return; }
  if (cardAmount > amount) { showToastAdmin('⚠️ Сумата с карта не може да е по-голяма от общата!'); return; }
  ...
  document.getElementById('shiftCardAmount').value = '';
...
async function loadCashbook() {
  ...
  if (shiftsList) {
    shiftsList.innerHTML = (allShifts || []).length === 0
      ...
      : (allShifts || []).map(s => {
          const card = parseFloat(s.card_amount || 0);
          const cash = parseFloat(s.amount) - card;
          return `
        <div class="cashbook-row">
          <div>
            ...
            <div class="cr-meta" style="margin-top:2px">💵 В брой: €${cash.toFixed(2)} · 💳 С карта: €${card.toFixed(2)}</div>
          </div>
  ...
  const totalCard     = shifts.reduce((s, r) => s + parseFloat(r.card_amount || 0), 0);
  const totalCash     = totalRevenue - totalCard;
  ...
  setEl('bossCardTotal', totalCard, '#4a9eff');
  setEl('bossCashTotal', totalCash, '#2ed573');
  ...
  const days = {};
  shifts.forEach(s => {
    if (!days[s.date]) days[s.date] = { revenue: 0, expenses: 0, card: 0, shifts: [], expenses_list: [] };
    days[s.date].revenue += parseFloat(s.amount);
    days[s.date].card += parseFloat(s.card_amount || 0);
    days[s.date].shifts.push(s);
  });
  expenses.forEach(e => {
    if (!days[e.date]) days[e.date] = { revenue: 0, expenses: 0, card: 0, shifts: [], expenses_list: [] };
  ...
  if (dailyList) {
    dailyList.innerHTML = sortedDays.length === 0
      ...
      : sortedDays.map(date => {
          ...
          const cashPart = d.revenue - d.card;
          ...
          return `<div class="day-card">
            ...
            <div class="day-detail">
              ...
              <span style="color:#2ed573">💵 В брой: €${cashPart.toFixed(2)}</span>
              <span style="color:#4a9eff">💳 С карта: €${d.card.toFixed(2)}</span>
      ...
      : shifts.map(s => {
        ...
        <div class="cashbook-row">
          <div>
            ...
            <div class="cr-meta" style="margin-top:2px">💵 €${cash.toFixed(2)} · 💳 €${card.toFixed(2)}</div>
          </div>
          <div class="cr-amount green">+€${parseFloat(s.amount).toFixed(2)}</div>
        </div>`;
        }).join('');
