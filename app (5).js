...
function buildTimeOptions() {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    // Не показвай часове след 22:50
    if (h > 22 || (h === 22 && m > 50)) break;
    const opt = document.createElement('option');
    opt.value = `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}`;
    opt.textContent = `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}` + (i === 0 ? ' (най-рано)' : '');
    sel.appendChild(opt);
  }
}
...
async function submitOrder() {
  ...
  // Генерирай уникален номер - комбинация от час и случайно число
  const orderNumber = '#' + new Date().getHours().toString().padStart(2,'0') + 
